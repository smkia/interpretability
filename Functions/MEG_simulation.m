...
for i = 1 : trialNumPerClass
    s{i} = meanpower(freq(1)) / meanpower(1) * peak (trialLength, 1, samplingRate, freq(1), 45, jitter) ...
        + noise(trialLength, 1, samplingRate) - meanpower(freq(2))/ meanpower(1) * peak (trialLength, 1, samplingRate,  freq(2), 75, jitter); 
end
effect{1} = s;
for i = 1 : trialNumPerClass
    s{i} = noise(trialLength, 1, samplingRate); %0.01*randn(1,250); %amp.*sin(2*pi*freq*t + phi*2*pi + 
end
effect{2} = s;
...
cfg.dip.pos             = sourcePos; %cm
cfg.dip.mom             = sourceMom';
cfg.dip.signal          =  meanpower(freq(1)) / meanpower(1) * peak (trialLength, 1, samplingRate, freq(1), 45, 0) ...
                            - meanpower(freq(2))/ meanpower(1) * peak (trialLength, 1, samplingRate, freq(2), 75, 0);
